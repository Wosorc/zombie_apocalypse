...
public partial class LocalControlWidget
{
	...
	/// <summary>
	/// A visual diff comparison window showing changes between file revisions.
	/// Features include synchronized scrolling, line-level character highlighting, and optional word wrapping.
	/// </summary>
	public class DiffWindow : Window
	{
		...
		{
			_fileName = fileName;
			_oldContent = oldContent ?? "[No previous version]";
			_newContent = newContent ?? "[No current version]";

			string escapedFileName = System.Security.SecurityElement.Escape( fileName );
			WindowTitle = $"Diff: {escapedFileName}";
		...
		private async void RefreshDiffView()
		{
			try
			{
				...
				header.Add(
					new Label("<b><font color='#ff7777'>  - PREVIOUS SNAPSHOT</font></b>"),
					1
				);
				header.Add(new Label("<b><font color='#77ff77'>  + CURRENT VERSION</font></b>"), 1);

				var loadingLabel = new Label("<i>Computing diff, please wait...</i>");
				_contentArea.Layout.Add(loadingLabel);

				var compareRow = _contentArea.Layout.AddRow();
				compareRow.Spacing = 10;

				var (oldHtml, newHtml) = await Task.Run(() =>
					GenerateSplitDiffHtml(_oldContent, _newContent)
				);
			...
			{
				Log.Error($"Failed to refresh diff view: {e.Message}");
				var errorLabel = new Label($"Error generating diff: {e.Message}");
				_contentArea.Layout.Add(errorLabel);
			}
		}
		...
		private (string oldHtml, string newHtml) GenerateSplitDiffHtml( string oldText, string newText )
{
	...
	var oldRes = new System.Text.StringBuilder( tableBase );
	var newRes = new System.Text.StringBuilder( tableBase );
	...
	for ( int i = 0; i < diffs.Count; i++ )
	{
		...
		if ( _lineLeveling && d.Type == DiffType.Delete && i + 1 < diffs.Count && diffs[i + 1].Type == DiffType.Add )
		{
			...
			oldRes.Append( $"<tr style='background-color: #321e1e;'><td style='{gutter}'>{oPtr++}</td><td style='{content}'>{hOld}</td></tr>" );
			newRes.Append( $"<tr style='background-color: #12261e;'><td style='{gutter}'>{nPtr++}</td><td style='{content}'>{hNew}</td></tr>" );
			i++;
			continue;
		}
		...
		{
			oldRes.Append( $"<tr><td style='{gutter}'>{oPtr++}</td><td style='{content}'>{esc}</td></tr>" );
			newRes.Append( $"<tr><td style='{gutter}'>{nPtr++}</td><td style='{content}'>{esc}</td></tr>" );
		}
		else if ( d.Type == DiffType.Add )
		{
			if ( _lineLeveling ) oldRes.Append( spacerRow );
			newRes.Append( $"<tr style='background-color: #12261e;'><td style='{gutter}'>{nPtr++}</td><td style='{content}'>{esc}</td></tr>" );
		}
		else if ( d.Type == DiffType.Delete )
		{
			oldRes.Append( $"<tr style='background-color: #321e1e;'><td style='{gutter}'>{oPtr++}</td><td style='{content}'>{esc}</td></tr>" );
			if ( _lineLeveling ) newRes.Append( spacerRow );
		}
	}
	...
	return (oldRes.ToString(), newRes.ToString());
}
		...
		private List<LineDiff> ComputeLineDiff( string[] oldLines, string[] newLines )
		{
			...
			for ( int i = 1; i <= n; i++ )
				for ( int j = 1; j <= m; j++ )
					lp[i, j] = (oldLines[i - 1] == newLines[j - 1]) ? lp[i - 1, j - 1] + 1 : Math.Max( lp[i - 1, j], lp[i, j - 1] );
