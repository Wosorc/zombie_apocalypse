...
/// <summary>
/// Git Version Control widget for s&box editor.
/// Provides UI for repository management, file change tracking, commit history, and remote synchronization.
/// </summary>
[Dock( "Editor", "Version Control", "account_tree" )]
public partial class LocalControlWidget : Widget
{
	private const int MAX_FILES_DISPLAYED = 50;
	private const float REFRESH_DEBOUNCE_TIME = 0.5f;
	private const int FILE_WATCHER_BUFFER_SIZE = 65536;
	...
	private void BuildChangesArea()
	{
		...
		_commitMessage.PlaceholderText = "Enter commit message... (Leave blank for default)";
	...
	private bool AddToGitIgnore( string entry )
	{
		...
		{
			Log.Error( $"Failed to add to .gitignore: {e.Message}" );
			return false;
		}
	}
	...
	private void RefreshHistoryUI()
	{
		...
		{
			Log.Error( $"Failed to refresh history UI: {e.Message}" );
		}
	}
	...
	private void OnRefreshClicked()
	{
		...
		{
			Log.Error( $"Failed to refresh changes UI: {e.Message}" );
		}
	}
	...
	private void SetupAutoRefresh()
	{
		...
		_watcher.InternalBufferSize = FILE_WATCHER_BUFFER_SIZE;
	...
	{
		Log.Warning($"Version Control Watcher error: {e.GetException().Message}. Restarting...");
	...
	{
		if (_needsRefresh && _lastRefresh > REFRESH_DEBOUNCE_TIME && !_gitOperationInProgress)
		{
			_needsRefresh = false;
			_lastRefresh = 0;
